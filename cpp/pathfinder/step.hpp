  ...
  bool GridPathFinder::nextGenSteps(int givenBatchSize = -1){
    ...
    while(givenBatchSize--){
      ...
      for(auto &fwd : steps[stepCnt]->fwdActions){
        ...
        if(cellVal != -1){  // && myUI.canvases[statics_to_obj[dest]].valType=="float"
          ...
          cur.second = std::min(cur.second, cellVal);
          sim.bounds[dest] = cur;
        }
        ...
        if(!coordIsEqual({x, y}, {-1, -1}) || command == EraseCanvas){
          if(sim.activeCanvas.find(dest) == sim.activeCanvas.end())
            //sim.activeCanvas[dest] = makeGrid(gridHeight, gridWidth);
            sim.activeCanvas[dest] = std::unordered_map<coord_t, int, CoordIntHash>();
        }
        ...
        if(command == DrawSinglePixel){
          ...
          sim.activeCanvas[dest] = makeGrid(h, w); */
          ...
          std::cout<<"Erasing: "<<infoTableRowIndex<<std::endl;
          ...
          if(!toHighlight) infoTableRowIndex *= -1;
          std::cout<<"To insert: "<<infoTableRowIndex<<std::endl;
          ...
          if(it != sim.edges[dest].end()) sim.edges[dest].erase(it);
      ...
      if(stepCnt % stateFreq == 0){
        ...
        if(stateNum % 100 == 0) std::cout<<"State "<<stateNum<<std::endl;
        if(!genStates) continue;
        std::unique_ptr<State> nextState = std::make_unique<State>();

        // canvas
        for(const auto p : sim.activeCanvas){
          //nextState->canvases[p.first] = deepCopyGrid(p.second);
