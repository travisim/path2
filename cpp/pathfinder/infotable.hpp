...
class InfoTable {
public:
    InfoTable(std::string tableIdentifier, int rowSize) : rowSize(rowSize) {
    ...
    int setHighlightAtIndex(int rowIndex) {
        ...
        std::cout<<"HIGHLIGHTROW: "<<highlightRow<<std::endl;
    ...
    std::vector<std::string> eraseRowAtIndex(int rowIndex) {
        ...
        if(highlighted) values.push_back(std::to_string(highlightRow));
        else values.push_back("-1");
        if(rowIndex+1<highlightRow)highlightRow--;
        else if(rowIndex+1==highlightRow) highlightRow = -1;
    ...
    void peek(){
        ...
        for(int i = 0; i < rows.size(); i++){
            ...
            if(i + 1 == highlightRow) std::cout<<"true";
            std::cout<<std::endl;
        }
        std::cout<<"\nEND OF PEEK\n\n";
